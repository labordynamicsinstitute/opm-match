...
Format and recode all OPM datas into comparable files. Created longitudinal variables of tenure to agency, posting length to duty station, tenure to occupation, quarter-over-quarter earnings change. 
...
The OPM datasets are merged iteratively starting with FOIA 2013 with FOIA 2016 (I). Then merge result(I) with Fedscope. Then merge result(II) with Buzzfeed. The 
...
3 separate merge files are created using FOIA 2013 as the base: FOIA 2013-FOIA 2016, FOIA 2013-Fedscope, and FOIA 2013-Buzzfeed.

Note: Merge m:m is used because a full outer join is not needed. It is fine to lose duplicates in this exercise. The merges are partitioned in to annual files due to the exponential increase to run time from file size.

## 05_binary_contribution.do

A CSV file showing the merge variables in 04 by importance in terms of contribution to uniquness and matching is generated. 
